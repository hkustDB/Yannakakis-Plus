...
drop view if exists semiJoinView4494821536451561544 CASCADE;
drop view if exists g3 CASCADE;
drop view if exists semiJoinView9206771152396330360 CASCADE;
drop view if exists semiEnum2217740493938547102 CASCADE;
drop view if exists semiEnum1624226747543115950 CASCADE;
create or replace view g1 as select Graph.src as v7, Graph.dst as v2, v8 from Graph, (SELECT src, COUNT(*) AS v8 FROM Graph GROUP BY src) AS c1 where Graph.src = c1.src and v8<2;create or replace view semiJoinView4494821536451561544 as select src as v2, dst as v4 from Graph AS g2 where (src) in (select v2 from g1);
...
/*+QUERY_TIMEOUT=86400000*/select sum(v2+v7+v6+v10+v8+v4) from semiEnum1624226747543115950;
