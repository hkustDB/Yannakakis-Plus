CREATE TABLE IF NOT EXISTS nation ( 
  ...
  `dummy` varchar   
) DISTRIBUTED BY BROADCAST;CREATE TABLE IF NOT EXISTS region ( 
  r_regionkey int NOT NULL COMMENT '',
  r_name varchar NOT NULL COMMENT '',
  r_comment varchar COMMENT '',
  dummy varchar
) DISTRIBUTED BY BROADCAST;CREATE TABLE IF NOT EXISTS part (
  ...
  dummy varchar
) DISTRIBUTED BY HASH  (p_partkey);CREATE TABLE IF NOT EXISTS supplier (  
  ...
  dummy varchar
) DISTRIBUTED BY HASH  (s_suppkey);CREATE TABLE IF NOT EXISTS partsupp (  
  ...
  dummy varchar
) DISTRIBUTED BY HASH  (ps_partkey);CREATE TABLE IF NOT EXISTS customer (  
  ...
  `dummy` varchar 
) DISTRIBUTED BY HASH  (c_custkey);CREATE TABLE IF NOT EXISTS orders (  
  ...
  dummy varchar
) DISTRIBUTED BY HASH  (o_orderkey);CREATE TABLE IF NOT EXISTS lineitem ( 
  ...
  dummy varchar
) DISTRIBUTED BY HASH  (l_orderkey);submit job insert overwrite into nation select * from oss_nation;
...
submit job insert overwrite into lineitem select * from oss_lineitem;ANALYZE TABLE customer UPDATE HISTOGRAM;
...
ANALYZE TABLE supplier UPDATE HISTOGRAM;create or replace view q2_inner as
