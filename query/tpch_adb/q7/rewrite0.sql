...
create or replace view aggView8171884648311358721 as select l_orderkey as v25, l_year as v9, l_suppkey as v1, l_extendedprice * (1 - l_discount) as v51, from lineitemwithyear as lineitemwithyear where l_shipdate>=DATE '1995-01-01' and l_shipdate<=DATE '1996-12-31';
...
create or replace view semiJoinView3013108551916751042 as select v25, v9, v1, v51 from aggView8171884648311358721 where (v1) in (select v1 from semiJoinView2223780679901324844);
...
create or replace view semiEnum5967720730526318280 as select distinct v1, v47, v51, v9 from semiEnum3581953715822531054 join semiJoinView3013108551916751042 using(v25);
create or replace view semiEnum484403286537607659 as select distinct v47, v51, v9, v4 from semiEnum5967720730526318280 join semiJoinView2223780679901324844 using(v1);
create or replace view semiEnum3083717252515600194 as select v43, v47, v51, v9 from semiEnum484403286537607659 join aggJoin8486327034885716142 using(v4);
select sum(v43), sum(v47), sum(v9), sum(v51) from semiEnum3083717252515600194;
