...
create or replace view aggView8752859015428585810 as select l_orderkey as v25, l_year as v9, l_suppkey as v1, l_extendedprice * (1 - l_discount) as v51 from lineitemwithyear as lineitemwithyear where l_shipdate>=DATE '1995-01-01' and l_shipdate<=DATE '1996-12-31';
...
create or replace view semiJoinView7588811530237693503 as select v25, v9, v1, v51 from aggView8752859015428585810 where (v25) in (select v25 from semiJoinView7445637448070817991);
...
create or replace view semiEnum7325740257583053348 as select distinct v25, v51, v43, v9 from semiEnum1741521260928797116 join semiJoinView7588811530237693503 using(v1);
create or replace view semiEnum6432078094351862122 as select distinct v43, v34, v51, v9 from semiEnum7325740257583053348 join semiJoinView7445637448070817991 using(v25);
create or replace view semiEnum334118978450405550 as select distinct v37, v43, v51, v9 from semiEnum6432078094351862122 join semiJoinView4771355823403436077 using(v34);
create or replace view semiEnum7137540933947025241 as select v43, v51, v9, v47 from semiEnum334118978450405550 join aggJoin8503338377088736481 using(v37);
select sum(v43), sum(v47), sum(v9), sum(v51) from semiEnum7137540933947025241;
