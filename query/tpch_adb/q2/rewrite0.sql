...
create or replace view s_new as select s_acctbal, s_name, s_address, s_phone, s_comment, s_suppkey, n_name from supplier, n_new where s_nationkey = n_nationkey;
create or replace view res as select distinct s_acctbal, s_name, n_name, p_partkey, p_mfgr, s_address, s_phone, s_comment from bag, s_new where s_suppkey = ps_suppkey;
