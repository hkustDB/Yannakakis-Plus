drop vie if exists bag1 cascade;
...
create or replace view l_agg as select l_orderkey, sum(l_quantity) as l_sum from lineitem where l_orderkey in (select v1_orderkey from q18_inner) group by l_orderkey;
