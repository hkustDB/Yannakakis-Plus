...
create or replace view l_new as select sum(l_extendedprice * (1 - l_discount)) as agg, s_nationkey, l_orderkey from lineitem, supplier where l_suppkey = s_suppkey group by s_nationkey, l_orderkey;
create or replace view res2 as select n_name, sum(agg) as agg from o_new, l_new where l_orderkey = o_orderkey and s_nationkey = n_nationkey group by n_name;
select sum(n_name), sum(agg) from res2;