...
create or replace view c1 as select src, count(*) as cnt from google group by src; 
create or replace view f1 as select src, min(cnt) as mf1 from c1 group by src;
create or replace view f2 as select mf1, g1.* from google g1 join f1 using (src);
create or replace view f3 as select dst, min(mf1) as mf2 from f2 group by dst;
create or replace view f4 as select mf2, g2.* from google g2 join f3 on g2.src=f3.dst;
create or replace view f5 as select src, max(cnt) as mf3 from c1 c2 group by src;
create or replace view f6 as select mf3, g3.* from google g3 join f5 on g3.dst=f5.src;
