...
drop view if exists semiJoinView3809618021338682188 cascade;
drop view if exists g3 cascade;
drop view if exists semiJoinView2119258132045840866 cascade;
drop view if exists semiEnum699580046230468386 cascade;
drop view if exists semiEnum3123948514559099299 cascade;
...
CREATE OR REPLACE VIEW semiJoinView2119258132045840866 AS 
SELECT v2, v4 
FROM semiJoinView3809618021338682188 
WHERE EXISTS (
    SELECT 1 
    FROM g3 
    WHERE semiJoinView3809618021338682188.v4 = g3.v4
);
