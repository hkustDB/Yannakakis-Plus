create or replace view res as SELECT g1.src AS src, g1.dst AS via1, g3.src AS via2, g3.dst AS dst,
    c1.cnt AS cnt1, c2.cnt AS cnt2, c3.cnt AS cnt3, c4.cnt AS cnt4
FROM wiki AS g1, wiki AS g2, wiki AS g3,
    (SELECT src, COUNT(*) AS cnt FROM wiki GROUP BY src) AS c1,
    (SELECT src, COUNT(*) AS cnt FROM wiki GROUP BY src) AS c2,
    (SELECT src, COUNT(*) AS cnt FROM wiki GROUP BY src) AS c3,
...
/*+QUERY_TIMEOUT=86400000*/select sum(src+via1+via2+dst+cnt1+cnt2+cnt3+cnt4) FROM res;