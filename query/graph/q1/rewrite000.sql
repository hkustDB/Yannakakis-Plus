...
create or replace view orderView2 as select v2, v4, MFL, row_number() over (partition by v4 order by MFL) as rn from joinView1;
create or replace view minView2 as select v4, MFL as MFL2 from orderView2 where rn = 1;
create or replace view joinView2 as select v4, v6, v10, MFL2 from g3 join minView2 using(v4) where MFL2 < v10;

create or replace view order2_0 as select * from orderView2 where rn <= 64;
create or replace view res2_0 as select v2, v4, v6, v10, rn from joinView2 join order2_0 using(v4) where MFL < v10;
create or replace view border0 as select v4, v6, v10 from res2_0 where rn = 64;
