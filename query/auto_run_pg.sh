            ...
            then
                ...
                then 
                    ...
                    do
                        ...
                        rm -f $OUT_FILE
                        touch $OUT_FILE
                        timeout -s SIGKILL 24h $PG "-d" "${DB}" "-p" "${port}" "-c" "\timing" "-f" "${SUBMIT_QUERY}" | grep "Time: " | tail -n 1 >> $OUT_FILE
                        status_code=$?
                    ...
                    do
                        ...
                        rm -f $OUT_FILE
                        touch $OUT_FILE
                        timeout -s SIGKILL 24h $PG "-d" "${DB}" "-p" "${port}" "-f" "${SUBMIT_QUERY_1}" "-c" "\timing" "-f" "${SUBMIT_QUERY_2}" | grep "Time: " | tail -n 1 >> $OUT_FILE
                        status_code=$?
                        if [[ ${status_code} -eq 137 ]]; then
                           echo "PG task timed out." >> $LOG_FILE
                           break
